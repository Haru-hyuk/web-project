...
import { ArrowLeft, BookOpen, Check, CheckCircle, Plus, Star, BarChart3, PlayCircle, BookMarked } from "lucide-react";
      ...
      <div className="detail-page">
        ...
        <div className="detail-grid">
          <div className="detail-left-col">
            ...
            {/* 학습 통계 & 빠른 액션 카드 */}
            <section className="detail-card study-stats-section">
              <div className="card-label">
                <BarChart3 size={16} />
                <span>학습 통계</span>
              </div>
              ...
              ) : studyLog && (studyLog.totalCorrect || 0) + (studyLog.totalWrong || 0) > 0 ? (
                <div className="study-stats-content">
                  <div className="stats-grid">
                    <div className="stat-item">
                      <div className="stat-label">정답</div>
                      <div className="stat-value stat-value--correct">
                        {studyLog.totalCorrect || 0}회
                      </div>
                    </div>
                    <div className="stat-item">
                      <div className="stat-label">오답</div>
                      <div className="stat-value stat-value--wrong">
                        {studyLog.totalWrong || 0}회
                      </div>
                    </div>
                  </div>

                  {(studyLog.totalCorrect || 0) + (studyLog.totalWrong || 0) > 0 && (() => {
                    const total = (studyLog.totalCorrect || 0) + (studyLog.totalWrong || 0);
                    const accuracy = total > 0 
                      ? Math.round(((studyLog.totalCorrect || 0) / total) * 100)
                      : 0;
                    ...
                    if (accuracy >= 80) {
                      ...
                      barStyle.background = "linear-gradient(90deg, var(--success-200) 0%, var(--success-500) 100%)";
                    } else if (accuracy >= 50) {
                      // 50-79%: 주황색 (주의)
                      barClass += " accuracy-bar--warning";
                      textClass += " accuracy-text--warning";
                      barStyle.background = "linear-gradient(90deg, var(--warning-200) 0%, var(--warning-500) 100%)";
                    } else {
                      // 50% 미만: 빨간색 (위험)
                      barClass += " accuracy-bar--danger";
                      textClass += " accuracy-text--danger";
                      barStyle.background = "linear-gradient(90deg, var(--danger-200) 0%, var(--danger-500) 100%)";
                    }
                    
                    return (
                      <div className="accuracy-section">
                        <div className="accuracy-label">정답률</div>
                  ...
                  {studyLog.lastStudyAt && (
                    <div className="last-study-date">
                      마지막 학습:{" "}
                      {new Date(studyLog.lastStudyAt).toLocaleDateString(
                        "ko-KR",
                        {
                          year: "numeric",
                          month: "long",
