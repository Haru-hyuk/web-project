...
const DashboardPage = () => {
  ...
  // 이번 주 데이터만 필터링하고 날짜 순으로 정렬
  const weeklyDataFiltered = (weeklyStudyData ?? []).filter((item) => {
    const itemDate = new Date(item.date);
    return itemDate >= thisMonday && itemDate <= thisSunday;
  });

  const weeklyDataSorted = [...weeklyDataFiltered].sort(
    (a, b) => new Date(a.date) - new Date(b.date)
  );

  // 차트 데이터를 월요일부터 시작하도록 재정렬
  // 요일 기준으로 정렬: 월(1) ~ 일(0) 순서
  const chartDataReordered = [...weeklyDataSorted].sort((a, b) => {
    const dateA = new Date(a.date);
    const dateB = new Date(b.date);
    const weekdayA = dateA.getDay(); // 0: 일 ~ 6: 토
    const weekdayB = dateB.getDay();
    
    // 월요일을 0으로 만들기 위해 변환 (일=6, 월=0, 화=1, ..., 토=5)
    const adjustedA = weekdayA === 0 ? 6 : weekdayA - 1;
    const adjustedB = weekdayB === 0 ? 6 : weekdayB - 1;
    
    // 같은 요일이면 날짜 순으로 정렬 (안전장치)
  ...
  const chartData = chartDataReordered.map((d) => ({
...
weeklyDataSorted.forEach((day) => {
  ...
  // 월요일을 0으로 만들기 위해 변환 (일=6, 월=0, 화=1, ...)
  const adjustedWeekday = weekday === 0 ? 6 : weekday - 1;
  attendanceByWeekday[adjustedWeekday] = day.learnedCount > 0;
});
            ...
            {wrongWordsList.length === 0 ? (
              ...
              wrongWordsList.map((item, index) => (
                <li
                  ...
                  className={`wrong-item ${item.wordId ? "wrong-item--clickable" : ""}`}
                  onClick={() => {
                    if (item.wordId) {
                      navigate(`/words/${item.wordId}`);
                    }
                  }}
                  style={item.wordId ? { cursor: "pointer" } : {}}
                >
                  <span
