...
const StoryDetailPage = () => {
  ...
  const handleWordClick = (item) => {
    const wordId = typeof item === "object" ? item.wordId : null;
    
    if (wordId) {
      navigate(`/words/${wordId}`);
    } else {
      // wordId가 없으면 단어 텍스트로 검색 페이지로 이동하거나 경고 표시
      const text = typeof item === "string" ? item : item?.text ?? item?.word ?? "";
      if (text) {
        // 단어 검색 페이지가 있다면 이동, 없으면 경고만 표시
        console.warn("단어 ID가 없어 상세 페이지로 이동할 수 없습니다:", text);
        // 또는 검색 페이지로 이동: navigate(`/words?search=${encodeURIComponent(text)}`);
      }
    }
  };
  ...
  return (
    <div className="page-container">
            ...
            <div className="vocab-list">
              {wordsLoading ? (
                ...
                words.map((item, i) => {
                  ...
                  const wordId = typeof item === "object" ? item.wordId : null;
                  const isClickable = !!wordId;

                  return (
                    <div
                      key={i}
                      className={`mini-word-card ${isClickable ? "mini-word-card--clickable" : ""}`}
      ...
      <ConfirmModal
        ...
        itemName={story?.titleKo || story?.title}
        confirmText="삭제"
        cancelText="취소"
        variant="danger"
      />
