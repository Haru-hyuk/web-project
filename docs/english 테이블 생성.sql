...
COMMENT ON TABLE "USER" IS '유저 정보';
COMMENT ON COLUMN "USER".USER_ID IS '유저 고유번호 (PK)';
COMMENT ON COLUMN "USER".USER_PW IS '비밀번호';
COMMENT ON COLUMN "USER".NICKNAME IS '닉네임 (고유)';
COMMENT ON COLUMN "USER".USER_NAME IS '유저 이름';
COMMENT ON COLUMN "USER".USER_BIRTH IS '유저 생년월일';
COMMENT ON COLUMN "USER".EMAIL IS '이메일 (고유)';
COMMENT ON COLUMN "USER".PREFERENCE IS '선호 분야';
COMMENT ON COLUMN "USER".GOAL IS '학습 목표';
COMMENT ON COLUMN "USER".DAILY_WORD_GOAL IS '하루 목표 단어 수';
COMMENT ON COLUMN "USER".CREATED_AT IS '가입일';
COMMENT ON COLUMN "USER".UPDATED_AT IS '업데이트일';
...
CREATE TABLE "WORD" (
    ...
    SYNONYMS    VARCHAR2(500)       NULL, -- 유의어 목록 (추가됨)
    ANTONYMS    VARCHAR2(500)       NULL, -- 반의어 목록 (추가됨)
...
COMMENT ON COLUMN "WORD".EXAMPLE_SENTENCE IS '예문';
COMMENT ON COLUMN "WORD".SYNONYMS IS '유의어 목록 (콤마 등으로 구분)';
COMMENT ON COLUMN "WORD".ANTONYMS IS '반의어 목록 (콤마 등으로 구분)';
COMMENT ON COLUMN "WORD".CATEGORY IS '분야';
COMMENT ON COLUMN "WORD"."LEVEL" IS '난이도';
...
ALTER TABLE "WORD_RELATION" ADD CONSTRAINT PK_WORD_RELATION PRIMARY KEY (CLUSTER_ID);

COMMENT ON TABLE "WORD_RELATION" IS '단어 관계 정보';
COMMENT ON COLUMN "WORD_RELATION".CLUSTER_ID IS '클러스터 고유번호 (PK)';
COMMENT ON COLUMN "WORD_RELATION".USER_ID IS '유저 ID (FK)';
COMMENT ON COLUMN "WORD_RELATION".RELATED_WORD_ID IS '연관 단어 고유번호 (FK)';
COMMENT ON COLUMN "WORD_RELATION".CENTER_WORD_ID IS '중심 단어 고유번호 (FK)';
COMMENT ON COLUMN "WORD_RELATION".SCORE IS '연관 강도(옵션)';
COMMENT ON COLUMN "WORD_RELATION".RELATION_TYPE IS '관계 유형(동의어/반의어/관련어/연어)';
COMMENT ON COLUMN "WORD_RELATION".CREATED_AT IS '생성일';
...
ALTER TABLE "FAVORITE_WORD" ADD CONSTRAINT FK_FAV_WORD_USER FOREIGN KEY (USER_ID) REFERENCES "USER" (USER_ID);
...
ALTER TABLE "WRONG_ANSWER_LOG" ADD CONSTRAINT FK_WRONG_LOG_USER FOREIGN KEY (USER_ID) REFERENCES "USER" (USER_ID);

-- STUDY_LOG FKs
ALTER TABLE "STUDY_LOG" ADD CONSTRAINT FK_STUDY_LOG_WORD FOREIGN KEY (WORD_ID) REFERENCES "WORD" (WORD_ID);
ALTER TABLE "STUDY_LOG" ADD CONSTRAINT FK_STUDY_LOG_USER FOREIGN KEY (USER_ID) REFERENCES "USER" (USER_ID);

-- WRONG_ANSWER_STORY FKs
ALTER TABLE "WRONG_ANSWER_STORY" ADD CONSTRAINT FK_STORY_USER FOREIGN KEY (USER_ID) REFERENCES "USER" (USER_ID);
...
-- WORD_RELATION FKs
ALTER TABLE "WORD_RELATION" ADD CONSTRAINT FK_REL_USER FOREIGN KEY (USER_ID) REFERENCES "USER" (USER_ID);
ALTER TABLE "WORD_RELATION" ADD CONSTRAINT FK_REL_RELATED_WORD FOREIGN KEY (RELATED_WORD_ID) REFERENCES "WORD" (WORD_ID);
ALTER TABLE "WORD_RELATION" ADD CONSTRAINT FK_REL_CENTER_WORD FOREIGN KEY (CENTER_WORD_ID) REFERENCES "WORD" (WORD_ID);

-- REFRESH_TOKEN FKs
ALTER TABLE "REFRESH_TOKEN" ADD CONSTRAINT FK_REFRESH_TOKEN_USER FOREIGN KEY (USER_EMAIL) REFERENCES "USER" (EMAIL);
...
-- 5.7. WORD_RELATION.CLUSTER_ID (PK)
CREATE OR REPLACE TRIGGER TRG_CLUSTER_ID
BEFORE INSERT ON "WORD_RELATION"
FOR EACH ROW
BEGIN
